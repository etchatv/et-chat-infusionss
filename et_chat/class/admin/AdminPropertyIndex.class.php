	...
	*/
	public function __construct (){ 
		...
		if ($_SESSION['etchat_v3_user_priv']=="admin"){
			...
			while($files = readdir($handle))
			{
				if($files != "." && $files != "..")
				{
					if (is_dir("styles/".$files) && $files!="admin_tpl" && $files!="install_tpl") {
                        if ($_SESSION['etchat_v3_style']==$files) $print_styles.= "<option value=\"".$files."\" selected>".$files."</option>\n";
			...
			while($files = readdir($handle))
			{
				if (!is_dir("lang/".$files) && ereg(".xml", $files)) {

					$xml_file = file_get_contents('lang/'.$files);
					$p = new XMLParser($xml_file);
					$p->Parse();
					if ($files == $_SESSION['etchat_v3_lang_xml_file']) $print_lang_files.= "<option value=\"".$files."\" selected>".$p->document->tagAttrs['lang']."</option>";
